...
func validateStep(ctx context.Context, s Step, names sets.String) (errs *apis.FieldError) {
	...
	for j, vm := range s.VolumeMounts {
		// Normalize the mount path before prefix checks so that path traversal
		// attempts like "/tekton/home/../results" cannot bypass the /tekton/
		// reservation (CVE-2026-40923).
